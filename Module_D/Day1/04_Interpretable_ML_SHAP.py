...
# MAGIC %md
# MAGIC Now, I can also delete the unneeded features like the name (after extracting the title from it), the ticket ID, the passenger ID.
...
# MAGIC %md
# MAGIC As a final step, I will encode the categorical features into numerical:
...
# MAGIC %md
# MAGIC I will drop the “Survival” outcome variable from the x-dataset
...
# MAGIC %md
# MAGIC #### Building a Linear Model
# MAGIC Finally, I’m going to build the model. I will go with a simple logistic regression model since the goal here is to see how the features affect the outcome and not to obtain a high score in prediction.
...
# MAGIC + The plot shows that his “Sex” (being a male) and his “class” (being in the third class) was passing his survival rate down. 
# MAGIC + The plot also shows that the number of siblings (“SibSp) being 0 increased his chance slightly. It seems that people who were alone in the ship without family were able to run faster without distraction.
# MAGIC 
...
# MAGIC %md
# MAGIC As expected, she is a female in class 1 who paid a high fare. This gave her a higher chance of survival. Also, the fact that she was a bit old (for the standard of time) decreased her chance a little bit.

# COMMAND ----------

# MAGIC %md
# MAGIC You can add explanatory variables to the model, changing the shap_values parameter:

# COMMAND ----------

shap.initjs()
shap_display = shap.force_plot(explainer.expected_value, 
                               shap_values[idx:idx+10,:], 
                               X_test.iloc[idx:idx+10,:], 
                               #link="logit", 
                               matplotlib=True
                              )
display(shap_display)
...
shap.initjs()
shap_display = shap.plots.force(explainer.expected_value, shap_values)
display(shap_display)
