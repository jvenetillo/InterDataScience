...
# MAGIC %md
# MAGIC ### Introduction to Word Embeddings  
# MAGIC 
...
# MAGIC %md
# MAGIC Training word embeddings with Gensim couldn't be easier. The only thing we need is a corpus of sentences in the language of interest.  
# MAGIC For our experiments we're going to use the abstracts of all ArXiv papers in the category cs.CL (computation and language) that were published before mid-April 2021 — a total of around 25,000 documents.  
# MAGIC We tokenize these abstracts with [_spaCy_](https://spacy.io/).

# COMMAND ----------

!head -10 data/arxiv/arxiv.csv
...
# MAGIC %md
# MAGIC ### From word vectors to document vectors
# MAGIC 
# MAGIC https://radimrehurek.com/gensim/models/doc2vec.html
...
model2 = Doc2Vec(documents2, vector_size=100, window=5, min_count=1, workers=4)
...
docvector = model2.infer_vector(['this','is','a','new','example','document'])
docvector
